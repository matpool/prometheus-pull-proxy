...
var (
	listenPullAddress    = kingpin.Flag("web.listen-pull-address", "Address to listen on for prometheus pull requests.").Default(":9012").String()
	listenPostAddress    = kingpin.Flag("web.listen-post-address", "Address to listen on for client post requests.").Default(":28080").String()
...
func main() {
	...
	if err != nil {
		level.Error(logger).Log("msg", "Read ca cert file failed", "err", err)
		os.Exit(1)
	}
	pool.AppendCertsFromPEM(caCert)
	postSrv := &http.Server{
		Addr:    *listenPostAddress,
		Handler: handler.mux,
	...
	level.Info(logger).Log("msg", "Listening", "pull_address", *listenPullAddress, "post_address", *listenPostAddress)
	go func() {
		if err := http.ListenAndServe(*listenPullAddress, handler.proxy); err != nil {
			level.Error(logger).Log("msg", "Listening pull failed", "err", err)
			os.Exit(1)
		}
	}()

	if err := postSrv.ListenAndServeTLS(*tlsCert, *tlsKey); err != nil {
		level.Error(logger).Log("msg", "Listening post failed", "err", err)
		os.Exit(1)
	}
}
