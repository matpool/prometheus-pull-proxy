...
func NewCoordinator(logger log.Logger) (*Coordinator, error) {
	c := &Coordinator{
		waiting:   make(chan *http.Request, 20),
		responses: map[string]chan *http.Response{},
		known:     map[string]time.Time{},
		logger:    logger,
	}
...
func (c *Coordinator) getRequestChannel() chan *http.Request {
	return c.waiting
}
